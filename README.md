...
Every warning appears twice: as a `(warning: ...)` note in `message`, and as an entry in `warnings` (a list of strings) for callers that want to branch without parsing prose. **Warnings never turn a successful update into `success: false`** — by then Gmail has applied the write, and a failure report would invite a retry that duplicates it; structurally, nothing that runs after the write shares a `try` with the code that maps exceptions to failures. Every update therefore costs one extra proxy round-trip (the read-back) on top of the pre-update thread read.
