...
- `POST /bulk-actions` - Apply multiple operations to multiple emails (`mark_read`, `archive`, `trash`, `apply_label:NAME`; `trash` is one approval-gated proxy call per message, so N messages means N operator approvals; the request stops at the first proxy refusal and reports the remaining operations as not attempted)
