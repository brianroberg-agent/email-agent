...
- `POST /drafts/{draft_id}/update` - Update an existing draft (preserves the draft's current thread by default; pass `thread_id` to move it or `attach_to_thread: false` to detach). Cross-checks the result's actual draft id against the request and reads the draft back (`format=metadata`) to compare its stored Subject; every problem is reported in `warnings` (and as a `(warning: ...)` note in `message`) on a `success: true` response — post-write checks never fail an update that landed. `id_changed` flags a reissued id.
